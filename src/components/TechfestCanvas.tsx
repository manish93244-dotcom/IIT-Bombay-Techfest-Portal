...
export default function TechfestCanvas({
  ...
  useEffect(() => {
    ...
    // Scoped variables for safe cleanup after try/catch
    ...
    let denseCoreGeometry: THREE.IcosahedronGeometry | null = null;
    let denseCoreMaterial: THREE.MeshBasicMaterial | null = null;
    let equatorialRingGeo: THREE.RingGeometry | null = null;
    let equatorialRingMat: THREE.MeshBasicMaterial | null = null;
    let polarRingGeo: THREE.RingGeometry | null = null;
    let polarRingMat: THREE.MeshBasicMaterial | null = null;
    ...
    let onCanvasClick: (() => void) | undefined;
    ...
    try {
      ...
      // Clear container and append canvas (cleans up any older fallback div)
      container.innerHTML = '';
      ...
      coreGeometry = new THREE.IcosahedronGeometry(2.0, 2);
      ...
      denseCoreGeometry = new THREE.IcosahedronGeometry(1.98, 3);
      denseCoreMaterial = new THREE.MeshBasicMaterial({
      ...
      equatorialRingGeo = new THREE.RingGeometry(2.3, 2.35, 64);
      equatorialRingMat = new THREE.MeshBasicMaterial({
      ...
      polarRingGeo = new THREE.RingGeometry(2.1, 2.15, 64);
      polarRingMat = new THREE.MeshBasicMaterial({
      ...
      console.warn("Three.js/WebGL failed to initialize. Displaying standard sci-fi blueprint fallback grid.", e);
      // Create fallback styling directly inside container to keep app readable and look sci-fi
      const fallbackDiv = document.createElement('div');
      fallbackDiv.className = "absolute inset-0 flex flex-col items-center justify-center text-center p-6 bg-slate-950/40 select-none z-10 pointer-events-auto cursor-default";
      
      fallbackDiv.innerHTML = `
        <div class="max-w-md space-y-4 p-6 border border-cyan-500/20 bg-slate-950/90 backdrop-blur-md rounded-2xl shadow-2xl">
          <div class="inline-flex items-center justify-center w-12 h-12 rounded-full border border-cyan-500/30 bg-cyan-950/10 text-cyan-400">
          ...
          <h3 class="text-xs font-mono font-black tracking-widest uppercase text-white">Constellation Hologram Standby</h3>
          <p class="text-xs text-slate-400 leading-relaxed font-sans">
            Your browser or system has WebGL hardware acceleration disabled or unsupported. Explore complete festival modules, search events indexes, and book access passes using standard screen controls.
          </p>
          <div class="text-[9px] uppercase font-mono text-cyan-400 border border-cyan-500/20 bg-cyan-950/20 px-2 py-1.5 rounded tracking-widest font-bold">
            CONSTELLATION INDEX SEARCH FULLY OPERATIONAL
          </div>
        </div>
      `;
      container.innerHTML = '';
    ...
    return () => {
      ...
      if (onCanvasClick) container.removeEventListener('click', onCanvasClick);
      ...
      if (denseCoreGeometry) denseCoreGeometry.dispose();
      if (denseCoreMaterial) denseCoreMaterial.dispose();
      if (equatorialRingGeo) equatorialRingGeo.dispose();
      if (equatorialRingMat) equatorialRingMat.dispose();
      if (polarRingGeo) polarRingGeo.dispose();
      if (polarRingMat) polarRingMat.dispose();
